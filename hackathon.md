...
Judge demo account ready: judges@subzero.app holds 6 copied subscriptions with evidence and cancel actions (copied server-side from the owner's data, source emails stripped, connections never touched). Password lives in the vibeapps submission notes only. Temp copy script deleted and prod redeployed without it.
